...
ShallowTrackClustersProducerSlim::ShallowTrackClustersProducerSlim(const edm::ParameterSet& iConfig)
...
/*  produces<std::vector<int> > ( Suffix + "clusterIdx"    ); //link: on trk cluster --> general cluster info 
  ...
  produces <std::vector<int> >         ( Prefix + "upper"        );
  produces <std::vector<float> >        ( Suffix + "globalphi"    );
  //produces <std::vector<float> >         ( Prefix + "localMom"        );
  produces <std::vector<float> >         ( Prefix + "localMomZ"        );
...
void ShallowTrackClustersProducerSlim::
...
//cout << "in here 1" << endl;
  ...
  auto       upper        = std::make_unique<std::vector<int>>();
  //auto       localMom        = std::make_unique<std::vector<float>>();
  auto       localMomZ        = std::make_unique<std::vector<float>>();
...
//cout << "in here 4.03" << endl;
      for(unsigned h=0; h<2; h++) { //loop over possible Hit options (1D, 2D)
				...
				if(!matchedhit && h==1) continue; 
                                ...
                                cout << " h " << h << " matchedhit  " << matchedhit  << " hit2D " << hit2D << " hit1D " << hit1D << endl;

//cout << "in here 4.04" << endl;
				...
				if(hit2D)
                                {
                                    cout << "h " << " stereo  " << moduleV.stereo << " upper " << moduleV.upper << " detId " << id << " partner DetId  " << tTopo->partnerDetId(id).rawId()  << " r " << TMath::Sqrt(TMath::Power( tsos.globalPosition().x() ,2 ) + TMath::Power( tsos.globalPosition().y() ,2 ) ) << endl;
                                } 

//cout << "in here 4.05" << endl;
				//find if cluster was already assigned to a previous track
				auto already_visited = mapping.find(i);
//cout << "in here 4.051" << endl;
				int nassociations = 1;
//cout << "in here 4.052" << endl;
				if(already_visited != mapping.end()) {
					nassociations += already_visited->second.size();
//cout << "in here 4.053" << endl;
					for(size_t idx : already_visited->second) {
						trackmulti->at(idx)++;
//cout << "in here 4.054" << endl;
					}
					already_visited->second.push_back(ontrk_cluster_idx);
//cout << "in here 4.055" << endl;
				}
				else { //otherwise store this 
					std::vector<size_t> single = {ontrk_cluster_idx};
					mapping.insert( std::make_pair(i, single) );
//cout << "in here 4.056" << endl;
				}

//cout << "in here 4.067" << endl;
				...
				if( matchedhit && h==0)
                                {
                                    cout << "h " << " stereo  " << moduleV.stereo << " upper " << moduleV.upper << " r " << TMath::Sqrt(TMath::Power( tsos.globalPosition().x() ,2 ) + TMath::Power( tsos.globalPosition().y() ,2 ) ) << endl;
                                } 
...
*/
      ...
      upper->push_back(           moduleV.upper        );
      localMomZ->push_back(      (theStripDet->toLocal(tsos.globalMomentum())).z() );    
      ...
      //localMom->push_back(      (theStripDet->toLocal(tsos.globalMomentum())) );    
//cout << "in here 6" << endl;
      /*
...
/*  iEvent.put(std::move(clusterIdx        ), Suffix + "clusterIdx"  );
  ...
  iEvent.put(std::move(upper),       Prefix + "upper"        );
  ...
  //iEvent.put(std::move(localMom),         Prefix + "localMom"          );
  iEvent.put(std::move(localMomZ),         Prefix + "localMomZ"          );
...
bool ShallowTrackClustersProducerSlim::trackFilter(const reco::Track* trk)
{
...
ShallowTrackClustersProducerSlim::NearDigis::
...
ShallowTrackClustersProducerSlim::moduleVars::
moduleVars(uint32_t detid, const TrackerTopology* tTopo) {
  ...
  if( SiStripDetId::TIB == subdetid ) {
    ...
    //updown        = tTopo->tibUpper(detid);
    upper     = tTopo->isUpper(detid) ? 1 : -1;
  } else
